...
def create_vulnerability_report(data, sbom_file, name):
    filename = f"vulnerability_report_{name}.html"
    ...
    for item in data:
        ...
        dependency = graph.build_tree(sbom_file, component_name, version, False, False)
...
def parse_trivy(report_path):
    ...
    id_to_label, _, _ = graph.build_package_index(data, None, None)
